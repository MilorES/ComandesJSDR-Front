  ...
  if (!data) {
    return (
      <div className="p-2 sm:p-6 flex justify-center">
        <div className="bg-white shadow-lg rounded-xl border border-gray-200 w-full p-4 md:p-6">
    ...
    <div className="p-2 sm:p-4 md:p-6 flex justify-center"> 
      <div className="bg-white shadow-lg rounded-xl border border-gray-200 w-full p-4 md:p-6">
        ...
        <div className="overflow-x-auto pb-2">
          <table className="w-full border-collapse text-center text-xs md:text-[0.83rem]">
            <thead>
              <tr>
                {Object.values(EstatComanda).map((estat) => (
                  <th
                    key={estat}
                    className={`px-2 md:px-4 py-2 ${getEstatColor(estat)} font-medium rounded-t-lg whitespace-nowrap`}
                  >
                    {getEstatText(estat)}
                  </th>
                ))}
            ...
            <tbody>
              <tr>
                {Object.values(EstatComanda).map((estat) => (
                  <td
                    key={estat}
                    className="px-2 md:px-4 py-2 border-t border-gray-200 font-semibold whitespace-nowrap"
                  >
                    {resum[estat]}
                  </td>
                ))}
        ...
        <div className="mt-4 text-right text-xs md:text-sm text-gray-600"> 
