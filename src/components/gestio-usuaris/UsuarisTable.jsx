...
export default function UsuarisTable({ usuaris = [], onEdit, onDelete }) {
  ...
  return (
    <div className="overflow-x-auto bg-white rounded-lg shadow text-gray-800">
      ...
      {usuaris.length === 0 ? (
        // Contenido si no hay usuarios (mantenemos el estilo de mensaje vacío)
          ...
          <tbody>
            {usuaris.map((u) => (
              <tr key={u.id} className="border-b hover:bg-gray-100 transition-colors">
                ...
                <td className="py-3 px-4 whitespace-nowrap">
                  <span
                    className={`inline-block px-3 py-1 rounded-full text-sm font-semibold ${u.role === "Administrator"
