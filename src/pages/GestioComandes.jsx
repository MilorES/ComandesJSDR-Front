...
// --- Grid modificado para dar más espacio a la columna "Número" ---
const gridLayoutClasses = "grid grid-cols-[180px_1fr_120px] md:grid-cols-[200px_250px_1fr_150px]";
...
export default function GestioComandes() {
  ...
  const [expandedId, setExpandedId] = useState(null); // Para controlar fila expandida
                ...
                <div key={comanda.id}>
                  ...
                  >
                    ...
                    <div className="flex justify-end items-center">
                      <span
                        className={`w-3 h-3 rounded-full md:hidden ${getEstatColor(comanda.estat)}`}
                        title={getEstatText(comanda.estat)}
                      ></span>

                      <span
                        className={`hidden md:inline-block px-3 py-1 rounded-full text-sm font-medium text-right ${getEstatColor(comanda.estat)}`}
                      >
                  ...
                  {expandedId === comanda.id && (
                    <div className="bg-gray-50 px-4 py-2 rounded-b-md text-sm">
                      ...
                      {comanda.linies.map((l) => (
                        <div key={l.id} className="grid grid-cols-3 gap-4 py-1 text-gray-600">
